...
from model import LSTMModel, RNNModel, GRUModel, GRU_ATTModel, weight_init
import numpy as np
...
def run(args):
    ...
    def train(model, optimizer, criterion):
        ...
        for ind, datas in enumerate(train_loader):
            ...
            labels = []
            for i in bat:
                y_i = [0 for i in range(150)]
                y_i[intent2idx[datas["intent"][i[-2]]]] = 1
                labels.append(intent2idx[datas["intent"][i[-2]]])
                y.append(y_i)
            ...
            optimizer.step()
            e_l += loss.item()
        print(e_l/len(train_loader), e_a/len(datasets["train"]), e_a)
    ...
    def eval(model, criterion):
        ...
        with torch.no_grad():
            for ind, datas in enumerate(eval_loader):
                ...
                loss = criterion(predictions, y)
                e_l += loss.item()
        print(e_l/len(eval_loader), e_a/len(datasets["eval"]), e_a)
    ...
    #model = RNNModel(300, hidden_size, n_layers, dropout, bid, 150)
